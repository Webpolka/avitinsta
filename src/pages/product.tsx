...
function ProductView({ product }: ProductViewProps) {
  ...
  const seller = USERS_DATA.find((user) => user.id === product.seller.id);

  const displaySizes = product.size.map((s) => {
    if (s.system === "EU") return `EU ${s.value}`;
    if (s.system === "LETTER") return s.value;
    return "One Size";
  });

  const activeDelivery = product.delivery
    .map((item) => DELIVERY_DATA.find((d) => d.id === item.id))
    .filter((d): d is NonNullable<typeof d> => Boolean(d && d.enabled));

  const firstDeliveryId =
    activeDelivery.length > 0 ? activeDelivery[0].id : undefined;
        ...
        <div className="flex flex-col gap-5 lg:max-w-[495px]">
          ...
          {displaySizes && (
            <div className="flex flex-row items-center gap-5 mb-2">
              <span className="ag-h7 font-medium text-grayscale-500 mb-2">
                Размер
              </span>
                ...
                {displaySizes.map(
                  (size) =>
                    size && (
                      <label
                        key={size}
                        className={`px-4 py-2 border border-solid ag-h7 font-medium cursor-pointer ${
                          cartItem.size === size
                            ? "border-black text-black"
                            : "border-[#d3d3d3] text-grayscale-500"
                        }`}
                      >
                        <input
                          type="radio"
