...
interface FilterProps {
  ...
  values: FiltersValue;
  onChange?: (newValues: FiltersValue) => void;
}

const Filter: React.FC<FilterProps> = ({ options, values, onChange }) => {
  ...
  const toggleArrayValue = (key: keyof FiltersValue, value: string) => {
    const current = values[key] as string[];
    const newArray = current.includes(value)
      ? current.filter((v) => v !== value)
      : [...current, value];
    onChange?.({ ...values, [key]: newArray });
  };

  const setPrice = (price: [number, number]) => {
    onChange?.({ ...values, priceMin: price[0], priceMax: price[1] });
  };

  const reset = () => {
    onChange?.({
      priceMin: options.priceMin ?? 1,
      priceMax: options.priceMax ?? 100000,
      sizes: [],
      brands: [],
      categories: [],
      conditions: [],
      ...
      <section className="mb-6 pb-4 border-b border-grayscale-100">
        ...
        {openSections.price && (
          <>
            ...
            <Slider.Root
              min={options.priceMin ?? 1}
              max={options.priceMax ?? 100000}
              step={500}
          ...
          >
            {options.sizes.map((size, index) => (
              <Checkbox
                ...
                className={
                  index % 2 === 0
                    ? "justify-start" // левая колонка
                    : "justify-between" // правая колонка
                }
              />
            ))}
          </div>
          ...
          <div className="grid gap-1">
            {options.conditions.map((cond) => (
              <Checkbox
                key={cond}
                label={cond}
                checked={values.conditions.includes(cond)}
                onChange={() => toggleArrayValue("conditions", cond)}
              />
            ))}
          </div>
        )}
...
const Checkbox: React.FC<CheckboxProps> = ({
  ...
  className,
    ...
    <label
      className={`cursor-pointer inline-flex items-center gap-4 select-none ${
        className || ""
      }`}
    >
      <input
      ...
      />
      <span className="cursor-pointer w-4.5 h-4.5 border border-black transition peer-checked:bg-black peer-checked:border-black" />
...
// const brandsList = ["Ric Owans", "11 By Boris Bidjan Saberi", "11 By Boris Bidjan Saberi", "11 By Boris Bidjan Saberi", "11 By Boris Bidjan Saberi", "11 By Boris Bidjan Saberi",  "Jordan", "Super boll", "violet", "Assus", "Sibur"];
// const categoriesList = ["Сумка", "Плац", "Клатч"];
