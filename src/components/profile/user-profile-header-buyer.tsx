...
export function UserProfileHeaderBuyer({
  user,
  onFollowToggle,
  ...
  return (
    <div className="flex flex-col gap-5 mb-7">
      ...
      {/* Личные фотографии (только для своего профиля) */}
      {user.photos && user.photos.length > 0 && (
        <div className="flex flex-row flex-wrap gap-4 sm:gap-7.5 mt-2">
          {user.photos.map((photo, index) => (
            <div
              key={index}
              className="w-20 sm:w-25 aspect-square rounded-xl bg-grayscale-100 overflow-hidden flex-wrap"
            >
              <img
                src={photo}
